...
def show_improve_phase():
    ...
    st.markdown("""
    **Objective:** To identify, test, and implement solutions that address the root causes from the Analyze phase. This is where we move from understanding the problem to actively solving it by finding the optimal settings for our critical process inputs (Xs).
    """)
    st.markdown("---")
...
def show_hybrid_strategy():
    ...
    guidance_data = pd.DataFrame({
        ...
        "Recommended Approach": [
            "**Classical Stats** (Hypothesis Testing, DOE)", "**ML + SPC** (Multivariate Anomaly Detection)", "**ML NLP** (Topic Modeling & Sentiment Analysis)", "**Classical DOE**", "**Hybrid:** ML Model + Bayesian Optimization", "**ML** (Real-time Predictive Model)"
        ],
        "Why?": [
